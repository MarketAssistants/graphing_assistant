...
class Graphing_Assistant: 
    ...
    def plot_histogram(self,data,title,flags,lines=[{},{}]): 
        ...
        if not(len_data == len_title and len_data == len_flags):
        ...
        for idx in range(len_data):
            ...
            if "y" in lines[idx].keys():
                plt.axhline(lines[idx]["y"], color=colors_list[color_idx+1], linestyle='solid', linewidth=2, label=f'Median y: {title[idx]["y"]:.2f}')
    ...
    def plot_xy_same(self,datax,datay,title,xlabel,ylabel,good_v_lines=[],bad_v_lines=[]): 
        ...
        for idx in range (len_datax):
            ...
            plt.plot(datax[idx], datay[idx], color = self.colors[idx%9],linestyle=self.linestyles[int(idx/10)],label = f"{ylabel[idx]}",linewidth=int(idx/10)+1,marker='o')
            # add mean and median lines
            # plt.axhline(0, color='purple', linestyle='dashed', linewidth=4)
            # plt.axhline(median_value, color='brown', linestyle='dashed', linewidth=4, label=f'Median y: {median_value:.2f}')
            plt.xticks(datax[idx])
